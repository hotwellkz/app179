...
import { collection, query, where, orderBy, onSnapshot, doc, writeBatch, limit, startAfter, getDocs } from 'firebase/firestore';
...
export const TransactionHistory: React.FC<TransactionHistoryProps> = ({
  ...
  const [isHistoryPasswordPromptOpen, setIsHistoryPasswordPromptOpen] = useState(
  ...
  useEffect(() => {
    if (!isOpen || (category.title === 'ЗП Сот.' && !isHistoryPasswordPromptOpen)) return;

    // Create initial query with limit
    let q = query(
    ...
    return () => unsubscribe();
  }, [category.id, isOpen, category.title, isHistoryPasswordPromptOpen]);
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      showErrorNotification('Ошибка при загрузке истории');
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg max-w-4xl w-full mx-4" style={{ maxHeight: '90vh' }}>
        ...
        <div 
          className="p-6 overflow-auto space-y-6" 
          {...handlers}
          onScroll={(e) => {
